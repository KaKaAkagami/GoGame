    ...
    if (m_diff == AIDifficulty::Easy) {
        ...
        for (auto [r, c] : legalMoves) {
            ...
            for (int k = 0; k < 4; ++k) {
                int nr = r + dr[k];
                int nc = c + dc[k];
                int v = child.getCell(nr, nc);
                if (v == aiColor) sameNeighbors++;
            }

            s += sameNeighbors * 0.5; // càng dính nhiều quân mình càng tốt
...
double GoAI::evaluatePosition(const GoGame& game, int aiColor) const
{
    ...
    // Thêm ưu tiên vị trí gần trung tâm
    int n = game.getBoardSize();
    int center = n / 2;
