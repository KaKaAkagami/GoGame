...
namespace
{
    ...
    // Stone skins (relative paths, NOT absolute D:\...)
...
void GameScreen::layout(const sf::Vector2u& winSize)
{
    ...
    // Theme from settings.cfg
    loadBoardTheme();
    applyBoardThemeToRect();

    // Stone theme from settings.cfg (NEW)
    loadStoneTheme();
    ...
    for (int i = 0; i < n; ++i)
    {
        for (int j = 0; j < n; ++j)
        {
            ...
            if (stoneSkinLoaded)
            {
                ...
                sf::Sprite sp(tex); // SFML3: ctor with texture (no default sprite needed)
